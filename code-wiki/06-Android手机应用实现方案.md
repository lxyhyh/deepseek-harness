...
| 内层沙箱 | **不启用** dsh 内层沙箱（bwrap/Landlock 手机内核大概率不支持）；隔离完全靠 chroot 边界 + 只读挂载（用户 2026-08-13 拍板） |
| 语音输入 | **第一版不做**，聊天工具栏不放语音按钮（用户 2026-08-13 拍板） |
| SDK/NDK 位置 | 装在**容器内** `/opt/android-sdk` 等目录；备份随"容器环境"一起，无需独立勾选（用户 2026-08-13 拍板） |
| 首次向导 API Key | 向导第 2 步改为**跳转 dsh 网页模型设置**填 Key，向导内不做表单（用户 2026-08-13 拍板） |
...
| 工具调用 | dsh 自带工具（fs/shell/web/subprocess 等）开箱即用；**Android 版 shell 不启用内层沙箱**（bwrap/Landlock 手机内核不支持），用 `dsh-bash-local`，隔离靠 chroot 边界 | 对齐 dsh 能力 seam；沙箱决策见 7.6 |
...
> **内层沙箱决策（2026-08-13，用户拍板）：** dsh 默认 shell 沙箱依赖 bwrap / Landlock，两者都要内核特性（Landlock 需 `CONFIG_SECURITY_LANDLOCK`，bwrap 需用户命名空间），Android 手机内核大概率不支持；且 dsh 的规则是"找不到可用沙箱就报错（SANDBOX_UNAVAILABLE），绝不静默裸奔"，会让 bash 工具在 Android 上**直接不可用**。因此 **Android 版不启用内层沙箱**：容器内 dsh 用 `dsh-bash-local`（无沙箱），隔离完全依赖 chroot 边界 + 只读挂载 `/sdcard/Download`。chroot 本身就是最外层隔离（AI 出不了容器、手机只读 Download），内层沙箱只是锦上添花；去掉后"跑 Linux 命令"始终可用。这是自用场景下的安全降级，明确为决策而非缺陷。
...
| 备份内容 | **可勾选打包**：容器环境 / 配置 / 会话记录 **三项**可勾，**默认只勾 配置+会话**（小快备份）；SDK/NDK 装在容器内，随"容器环境"一起备份，无独立"已装工具"勾选 |
...
| 页面 | 内容 | 关键交互 |
|---|---|---|
| ① 聊天主界面 | 原生顶栏（右上角**容器状态绿点** + 菜单按钮）+ **网页主体**（dsh Web UI：对话 + composer）；composer **输入框单独一行（在上）**，**工具栏一行（在下）**：模型/思考强度选择器 + 发文件 + 发送；顶部仅一条栏，不叠加 | 原生壳与网页主体通过 WebView 桥接（容器状态、文件发送）；网页主体内一切交互复用 dsh |
| ② 右上角菜单（抽屉） | 工具管理 / 镜像源 / 环境变量 / 备份恢复 / 设置 / 重启容器 / 更新工具 | 列出入口，点击跳对应页；"重启容器"为操作项、"更新工具"跳 ⑤ 工具管理页 |
| ③ 首次安装向导 | 多步引导：解包 rootfs → **跳转 dsh 网页设置（填 API Key）** → 启动服务 → 进入聊天 | 每步进度 + 失败可重试；Key 不在向导内做表单（见决策表）；完成后直达① |
...
- [ ] 实现"首次安装向导"：`su -c` 部署 chroot Ubuntu 24 rootfs + Node.js + dsh 核心（自动化脚本 + 引导 UI）；API Key 步骤**跳转 dsh 网页模型设置**，向导内不做表单
- [ ] 实现服务守护：`su -c` 启动 chroot 容器 + dsh（`dsh web`）、健康检查、停止、日志查看；用**前台服务 + 常驻通知 + 开机广播（BOOT_COMPLETED）**保住 `dsh web` 常驻、重启手机后自动拉起
...
| **dsh 内层沙箱（bwrap/Landlock）在 Android 不可用，bash 工具会直接报错** | **高** | **已决策：不启用内层沙箱**，容器内用 `dsh-bash-local`（无沙箱），隔离靠 chroot 边界 + 只读挂载（见 7.6）；"跑 Linux 命令"始终可用 |
| Android 系统杀后台，`dsh web` 常驻可能中断 | 中 | 用**前台服务 + 常驻通知 + 开机广播（BOOT_COMPLETED）**保住常驻、重启后自动拉起（阶段 1 服务守护） |
| dsh Web UI 是桌面端网页，手机 WebView 触控/布局可能不适配 | 低 | 阶段 0 真机验证 WebView 加载效果；必要时调整视口/缩放设置 |
| 云端 API 依赖网络 | 低 | 明确告知离线不可用；后续可加本地模型路线 |
| 构建沙箱缺 Android SDK（开发环境） | 中 | 已确认可安装；如需实际打包，先执行阶段 0 |
