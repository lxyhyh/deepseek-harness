...
| 页面 | 内容 | 关键交互 |
|---|---|---|
| ① 聊天主界面 | 原生顶栏（右上角**容器状态绿点** + 菜单按钮）+ **网页主体**（dsh Web UI：模型/思考强度选择器在**输入栏**、对话、composer 带附件/发送）；顶部仅一条栏，不叠加 | 原生壳与网页主体通过 WebView 桥接（容器状态、文件发送）；网页主体内一切交互复用 dsh |
...
| ④ 设置页 | **模型与密钥（入口，跳 dsh 网页主体设置）**、工作目录、服务端口、深色模式开关、工具下载源 | 下载源单选 + **测速按钮**（显示延迟 ms）+ 连通状态；模型入口跳网页主体设置 |
