...
### 7.8 应用 UI 设计方案（App 原生层，非 WebView）

> 聊天主体复用 dsh 自带 Web UI（不重写），所以 UI 工作集中在：**聊天壳 + 右上角菜单 + 设置 + 工具管理 + 环境变量 + 备份 + 首次安装向导**这几块原生页面。UI 设计是独立工作项，纳入阶段排期（见 8 阶段 1）。

**已确认的 UI 决策（用户 2026-08-13）：**

| 决策点 | 结论 |
|---|---|
| 主界面结构 | **单聊天界面**：打开 App 直接就是聊天；工具管理/设置等收进右上角菜单 |
| 视觉风格 | **Material 3 原生风格**，跟随系统动态色/字体，不做重定制 |
...
| 页面 | 内容 | 关键交互 |
|---|---|---|
| ① 聊天主界面 | 顶部状态条（容器运行中/停止、联网状态、右上角菜单按钮）+ 全屏 WebView（dsh Web UI） | 右上角菜单按钮 → 打开抽屉/弹层；WebView 内对话、工具卡片、会话历史复用 dsh |
| ② 右上角菜单（抽屉） | 工具管理 / 环境变量 / 备份恢复 / 设置 / 重启容器 / 更新工具 | 列出入口，点击跳对应页；"重启容器/更新工具"为操作项 |
| ③ 首次安装向导 | 多步引导：解包 rootfs → 填 API Key → 启动服务 → 进入聊天 | 每步进度 + 失败可重试；完成后直达① |
| ④ 设置页 | API Key、模型选择、工作目录、深色模式开关、服务端口 | 表单保存到 dsh 配置；深色开关即时生效 |
| ⑤ 工具管理页 | SDK/NDK 已装列表（版本/占用空间）、安装（推荐组合/自选版本）、卸载、arm64 替代来源标注 | 兼容性提示（见 7.2.3）、替代来源标黄（见 7.2.5） |
| ⑥ 环境变量编辑器 | 键/值列表，新增/修改/删除 | 改后提示"重启容器生效"（见 7.2.4） |
| ⑦ 备份/恢复页 | 一键备份、选择备份文件恢复、进度条 | 备份产物说明 + 换机引导（见 7.7） |

**UI 设计要点：**
- 全部原生 Compose + Material 3 组件（TopAppBar / NavigationDrawer / Card / Dialog / Snackbar），不引第三方 UI 库；
- 深色模式：`isSystemInDarkTheme()` + 手动覆盖的 `darkTheme` 状态（跟随/浅/深三态），主题色用 Material3 默认配色；
- 由于主界面是 WebView 全屏，原生 UI 只承担"壳 + 功能页"，首屏加载快；
- 全中文界面；面向非开发者的文案（如"正在安装 Linux 环境，约需几分钟"）。

**UI 设计工作拆分（独立排期）：**
1. 设计阶段（页面结构 + 交互流程 + 原型）：对应阶段 1 的 UI 设计任务；
2. 实现阶段（各页 Compose 编码）：随阶段 2 各功能页一起落地；
...
### 阶段 2：功能落地（各功能页均为原生 Compose UI，见 7.8 页面清单 ④–⑦）
- [ ] 设置页 UI：API Key / 模型 / 工作目录 / **深色模式开关** / 服务端口，写入 dsh 配置（cordis.yml 覆盖）
- [ ] 实现隔离边界：只读挂载 `/sdcard/Download`，验证其它手机目录不可访问
- [ ] 备份/恢复页 UI：一键备份/恢复（打包容器+配置+会话日志，可跨机恢复）
- [ ] 工具管理页 UI：查看/安装/卸载 SDK·NDK 等，内置版本兼容矩阵 + 自动推荐组合 + arm64 替代来源标注（见 7.2.2/7.2.3/7.2.5）
